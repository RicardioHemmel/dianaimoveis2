...
export function ResetPasswordForm({
  ...
  const tokenInvalid = !verifyResetTokenResponse.success;
        ...
        <form action={formAction} className="space-y-5">
          ...
          {tokenInvalid && (
            <p className="text-sm text-red-600">
              {verifyResetTokenResponse.message ?? "Token inválido ou expirado"}
            </p>
          )}
          ...
          <Button
            type="submit"
            disabled={pending || validationError.length > 0 || tokenInvalid}
